...
import com.example.smartmoviemock.viewmodel.NowPlayingFragmentViewModel
...
import com.example.smartmoviemock.databinding.FragmentNowPlayingBinding
...
class NowPlayingFragment : BaseFragment<FragmentNowPlayingBinding>() {
    ...
    private val nowPlayingFragmentViewModel: NowPlayingFragmentViewModel by viewModels()
    ...
    override fun getViewBinding(
        ...
        return FragmentNowPlayingBinding.inflate(
            inflater,
            container,
            false
        )
    ...
    override fun setObserver() {
        homeScreenViewModel.homeScreenMovieState.observe(viewLifecycleOwner) { state ->
            when (state) {
                is HomeScreenMovieState.Success -> {
                    ...
                    listMovies.clear()
                    listMovies.addAll(state.nowPlayingMovies)
        ...
        nowPlayingFragmentViewModel.nowPlayingMovieState.observe(viewLifecycleOwner) { state ->
            when (state) {
        ...
        nowPlayingFragmentViewModel.loadMoreMovieState.observe(viewLifecycleOwner) { state ->
            when (state) {
                is MovieLoadingState.Success -> {
                    listMovies.addAll(state.movies)
                    movieAdapter.submitList(listMovies)
    ...
    override fun initListener() {
        binding.swipeRefreshLayout.setOnRefreshListener {
            nowPlayingFragmentViewModel.fetchNowPlayingMovies()
        ...
        binding.nestedScrollViewMovies.setOnLoadMoreListener {
            nowPlayingFragmentViewModel.loadMoreNowPlayingMovies()
        }
    }
