...
import com.example.smartmoviemock.viewmodel.PopularFragmentViewModel
...
import com.example.smartmoviemock.databinding.FragmentPopularMovieBinding
...
class PopularMovieFragment : BaseFragment<FragmentPopularMovieBinding>() {
    ...
    override fun setObserver() {
        homeScreenViewModel.homeScreenMovieState.observe(viewLifecycleOwner) { state ->
            when (state) {
                is HomeScreenMovieState.Success -> {
                    ...
                    listMovies.clear()
                    listMovies.addAll(state.popularMovies)
        ...
        popularFragmentViewModel.popularMovieState.observe(viewLifecycleOwner) { state ->
            when (state) {
        ...
        popularFragmentViewModel.loadMoreMovieState.observe(viewLifecycleOwner) { state ->
            when (state) {
                is MovieLoadingState.Success -> {
                    listMovies.addAll(state.movies)
                    movieAdapter.submitList(listMovies)
    ...
    override fun initListener() {
        binding.swipeRefreshLayout.setOnRefreshListener {
            popularFragmentViewModel.fetchPopularMovies()
        ...
        binding.nestedScrollViewMovies.setOnLoadMoreListener {
            popularFragmentViewModel.loadMorePopularMovies()
        }
    }
