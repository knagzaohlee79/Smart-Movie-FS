...
import com.example.smartmoviemock.viewmodel.TopRatedFragmentViewModel
...
import com.example.smartmoviemock.databinding.FragmentTopRatedBinding
...
class TopRatedFragment : BaseFragment<FragmentTopRatedBinding>() {
    ...
    private val topRatedFragmentViewModel: TopRatedFragmentViewModel by viewModels()
    ...
    override fun getViewBinding(
        ...
        return FragmentTopRatedBinding.inflate(
            inflater,
            container,
            false
        )
    ...
    override fun setObserver() {
        homeScreenViewModel.homeScreenMovieState.observe(viewLifecycleOwner) { state ->
            when (state) {
                is HomeScreenMovieState.Success -> {
                    ...
                    listMovies.clear()
                    listMovies.addAll(state.topRatedMovies)
        ...
        topRatedFragmentViewModel.topRatedMovieState.observe(viewLifecycleOwner) { state ->
            when (state) {
        ...
        topRatedFragmentViewModel.loadMoreMovieState.observe(viewLifecycleOwner) { state ->
            when (state) {
                is MovieLoadingState.Success -> {
                    listMovies.addAll(state.movies)
                    movieAdapter.submitList(listMovies)
    ...
    override fun initListener() {
        binding.swipeRefreshLayout.setOnRefreshListener {
            topRatedFragmentViewModel.fetchTopRatedMovies()
        ...
        binding.nestedScrollViewMovies.setOnLoadMoreListener {
            topRatedFragmentViewModel.loadMoreTopRatedMovies()
        }
    }
